...
func Mount(app *router.AppDeps) {
	...
	app.RegisterProtectedRoutes(func(r chi.Router) {
		r.Route("/settings", func(r chi.Router) {
			...
			r.With(rbac.RequirePermission(app.RBACService, rbac.PermSettingsView)).Get("/backup-key/status", h.HandleGetBackupKeyStatus)
			r.With(rbac.RequirePermission(app.RBACService, rbac.PermSettingsManage)).Post("/backup-key/generate", h.HandleGenerateBackupKey)
			r.With(rbac.RequirePermission(app.RBACService, rbac.PermSettingsManage)).Post("/backup-key/install", h.HandleInstallBackupKey)
