...
// readHostStats reads CPU%, used memory, load averages, uptime, and root
// filesystem usage from /proc. Returns ok=false when the host filesystem does
// not expose the expected files.
func readHostStats() (HostStatsPayload, bool) {
	out := HostStatsPayload{}

	cpu, ok := readCPUPercent()
	if !ok {
		return out, false
	}
	out.CPUPercent = cpu
	...
	fsTotal, fsUsed, fsOK := readRootFSUsage()
	if fsOK {
		out.FSTotal = fsTotal
		out.FSUsed = fsUsed
	}
...
// readCPUPercent samples /proc/stat twice with a short delay and returns the
