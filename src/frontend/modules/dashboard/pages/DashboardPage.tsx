...
const WidgetGrid = lazy(() =>
  import("../components/WidgetGrid").then((m) => ({ default: m.WidgetGrid })),
);
const WidgetPicker = lazy(() =>
  import("../components/WidgetPicker").then((m) => ({
    default: m.WidgetPicker,
  })),
);
...
export default function DashboardPage() {
  ...
  const { editMode, setEnvironmentScope, setEditMode, resetToDefault } =
    useDashboardLayoutStore();
  const [pickerOpen, setPickerOpen] = useState(false);
  useWidgetSync();

  useEffect(() => {
    setEnvironmentScope(currentId);
  }, [currentId, setEnvironmentScope]);
