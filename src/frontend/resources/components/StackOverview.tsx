    ...
    queries: dockerEnvs.map((env) => ({
      ...
      queryFn: () =>
        api
          ...
          .then((r) =>
            (r.data ?? []).map((stack) => ({ ...stack, envId: env.id, envName: env.name })),
          ),
                ...
                {group.stacks.map((stack) => {
                  ...
                  return (
                    <Command.Item
                      ...
                      onSelect={() => {
                        navigate(`/stacks/${encodeURIComponent(stack.name)}?env=${group.envId}`);
                        onOpenChange(false);
                      }}
                    ...
                    >
                      ...
                      <span className="ml-auto flex shrink-0 items-center gap-2">
                        ...
                        <span className="text-xs text-muted-foreground">{stack.status}</span>
                      </span>
                    </Command.Item>
                  );
                })}
