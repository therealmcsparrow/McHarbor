...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      'fixed inset-0 z-50 bg-gray-900/50 dark:bg-neutral-900/80 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0',
      className
    )}
    {...props}
  />
...
const DialogHeader = ({ className, ...props }: HTMLAttributes<HTMLDivElement>) => (
  <div className={cn('shrink-0 border-b border-border px-4 py-3 sm:text-left', className)} {...props} />
);
...
const DialogFooter = ({ className, ...props }: HTMLAttributes<HTMLDivElement>) => (
  <div className={cn('shrink-0 flex items-center justify-end gap-x-2 border-t border-border px-4 py-3', className)} {...props} />
);

function isDialogSlot(child: ReactNode, slot: typeof DialogHeader | typeof DialogBody | typeof DialogFooter) {
...
>(({ className, children, ...props }, ref) => {
  ...
  const hasExplicitBody = childArray.some((child) => isDialogSlot(child, DialogBody));
  const structuredChildren = hasExplicitBody
    ? children
    : (
      <>
        {childArray.filter((child) => isDialogSlot(child, DialogHeader))}
        {childArray.some((child) => !isDialogSlot(child, DialogHeader) && !isDialogSlot(child, DialogFooter)) ? (
          <DialogBody>
            {childArray.filter((child) => !isDialogSlot(child, DialogHeader) && !isDialogSlot(child, DialogFooter))}
          </DialogBody>
        ) : null}
