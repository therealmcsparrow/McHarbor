...
Run a basic Docker container action
...
- `Action` (select, required): Choose one of the built-in options. Options: Start, Stop, Restart, Remove.
...
This frontend node is bound to the workflow action key `docker-action` in `src/backend/modules/workflows/service.go`.
Category: `action`.
