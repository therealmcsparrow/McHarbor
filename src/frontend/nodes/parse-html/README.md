...
Extract text, HTML, or attributes from HTML content
...
- `Selector` (text, optional): Single-line text value. Default: `body`.
- `Output Type` (select, optional): Choose one of the built-in options. Default: `text`. Options: Text, HTML, Attribute.
- `Attribute` (text, optional): Single-line text value.
...
This frontend node is bound to the workflow action key `parse-html` in `src/backend/modules/workflows/service.go`.
Category: `utility`.
