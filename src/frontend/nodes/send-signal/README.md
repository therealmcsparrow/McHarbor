...
Send a message through a Signal API bridge
...
- `Sender` (text, required): Single-line text value.
- `Recipient` (text, required): Single-line text value.
...
This frontend node is bound to the workflow action key `send-signal` in `src/backend/modules/workflows/service.go`.
