...
Run a host command through a temporary container
...
- `Command` (textarea, required): Multi-line text value for bodies, templates, or larger content.
- `Timeout (seconds)` (number, optional): Numeric value. Default: `30`.
...
This frontend node is bound to the workflow action key `host-exec` in `src/backend/modules/workflows/service.go`.
Category: `action`.
