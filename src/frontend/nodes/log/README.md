...
Write a message to the workflow log stream
...
- `Level` (select, optional): Choose one of the built-in options. Default: `info`. Options: Info, Warn, Error.
- `Message` (textarea, optional): Multi-line text value for bodies, templates, or larger content.
...
This frontend node is bound to the workflow action key `log` in `src/backend/modules/workflows/service.go`.
Category: `utility`.
