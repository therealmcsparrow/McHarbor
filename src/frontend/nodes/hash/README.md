...
Hash a value using a selected algorithm
...
- `Algorithm` (select, optional): Choose one of the built-in options. Default: `sha256`. Options: MD5, SHA1, SHA256, SHA512.
...
This frontend node is bound to the workflow action key `hash` in `src/backend/modules/workflows/service.go`.
Category: `utility`.
