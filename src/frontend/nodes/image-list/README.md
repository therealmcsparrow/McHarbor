...
- `Show All` (toggle, optional): Turns behavior on or off. Default: `False`.
- `Only Dangling` (toggle, optional): Turns behavior on or off. Default: `False`.
...
This frontend node is bound to the workflow action key `image-list` in `src/backend/modules/workflows/service.go`.
Category: `action`.
