...
Render a text template using values from the workflow message
...
- `Template` (textarea, required): Multi-line text value for bodies, templates, or larger content.
...
This frontend node is bound to the workflow action key `template` in `src/backend/modules/workflows/service.go`.
Category: `utility`.
