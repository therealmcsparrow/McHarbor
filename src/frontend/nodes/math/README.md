...
Apply a math operation to a numeric value
...
- `Operation` (select, optional): Choose one of the built-in options. Default: `add`. Options: Add, Subtract, Multiply, Divide, Round, Floor, Ceil, Absolute, Min, Max, Modulo.
- `Operand` (number, optional): Numeric value. Default: `0`.
...
This frontend node is bound to the workflow action key `math` in `src/backend/modules/workflows/service.go`.
Category: `utility`.
