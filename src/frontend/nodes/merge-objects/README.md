...
Merge multiple source objects into one output
...
- `Sources` (text, required): Single-line text value.
...
This frontend node is bound to the workflow action key `merge-objects` in `src/backend/modules/workflows/service.go`.
Category: `utility`.
