...
Assert that a condition is true and branch on the result
...
- Output `pass`: Sends the outgoing msg on this port.
- Output `fail`: Sends the outgoing msg on this port.
...
- `Failure Message` (text, optional): Single-line text value.
...
This frontend node is bound to the workflow action key `assert` in `src/backend/modules/workflows/service.go`.
Category: `logic`.
