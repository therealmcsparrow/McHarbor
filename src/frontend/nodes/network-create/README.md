...
- `Name` (text, required): Single-line text value.
- `Driver` (text, optional): Single-line text value. Default: `bridge`.
- `Internal` (toggle, optional): Turns behavior on or off. Default: `False`.
- `Subnet` (text, optional): Single-line text value.
...
This frontend node is bound to the workflow action key `network-create` in `src/backend/modules/workflows/service.go`.
Category: `action`.
