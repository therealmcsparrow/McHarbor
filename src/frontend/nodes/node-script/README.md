...
Run custom JavaScript against the current workflow message
...
- `Code` (code, required): Code or script content used by the node.
...
This frontend node is bound to the workflow action key `node-script` in `src/backend/modules/workflows/service.go`.
Category: `utility`.
