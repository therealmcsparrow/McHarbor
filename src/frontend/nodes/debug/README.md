...
Emit the current message to the debug stream
...
- `Show Full Message` (toggle, optional): Turns behavior on or off. Default: `False`.
...
This frontend node is bound to the workflow action key `debug` in `src/backend/modules/workflows/service.go`.
Category: `utility`.
