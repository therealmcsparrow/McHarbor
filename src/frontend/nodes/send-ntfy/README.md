...
- `Server URL` (text, optional): Single-line text value. Default: `https://ntfy.sh`.
- `Topic` (text, required): Single-line text value.
...
- `Priority` (text, optional): Single-line text value.
- `Tags` (text, optional): Single-line text value.
...
This frontend node is bound to the workflow action key `send-ntfy` in `src/backend/modules/workflows/service.go`.
