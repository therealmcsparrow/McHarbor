...
Send a WhatsApp message through an API
...
- `API URL` (text, required): Single-line text value.
- `Access Token` (text, required): Single-line text value.
- `Phone Number` (text, required): Single-line text value.
...
This frontend node is bound to the workflow action key `send-whatsapp` in `src/backend/modules/workflows/service.go`.
