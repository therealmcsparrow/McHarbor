...
Write message data to a file in the workflow data directory
...
- `Mode` (select, optional): Choose one of the built-in options. Default: `overwrite`. Options: Overwrite, Append.
- `Encoding` (select, optional): Choose one of the built-in options. Default: `utf-8`. Options: UTF-8, Base64.
...
This frontend node is bound to the workflow action key `write-file` in `src/backend/modules/workflows/service.go`.
Category: `utility`.
