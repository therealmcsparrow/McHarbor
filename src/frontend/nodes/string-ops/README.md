...
Apply common string operations to a value
...
- `Operation` (select, optional): Choose one of the built-in options. Default: `uppercase`. Options: Uppercase, Lowercase, Trim, Replace, Split, Join, Substring, Reverse.
- `Arg 1` (text, optional): Single-line text value.
- `Arg 2` (text, optional): Single-line text value.
...
This frontend node is bound to the workflow action key `string-ops` in `src/backend/modules/workflows/service.go`.
Category: `utility`.
