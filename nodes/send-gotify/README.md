...
- `Server URL` (text, required): Single-line text value.
- `App Token` (text, required): Single-line text value.
...
- `Priority` (number, optional): Numeric value. Default: `5`.
...
This frontend node is bound to the workflow action key `send-gotify` in `src/backend/modules/workflows/service.go`.
