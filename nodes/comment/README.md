...
Annotate a workflow without changing execution
...
- `Comment` (textarea, optional): Multi-line text value for bodies, templates, or larger content.
...
This frontend node is bound to the workflow action key `comment` in `src/backend/modules/workflows/service.go`.
Category: `utility`.
