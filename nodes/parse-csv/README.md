...
Parse CSV or TSV text into structured rows
...
- `Delimiter` (text, optional): Single-line text value. Default: `,`.
- `Has Headers` (toggle, optional): Turns behavior on or off. Default: `True`.
...
This frontend node is bound to the workflow action key `parse-csv` in `src/backend/modules/workflows/service.go`.
Category: `utility`.
