...
- `Tail Lines` (number, optional): Numeric value. Default: `100`.
- `Since` (text, optional): Single-line text value.
...
This frontend node is bound to the workflow action key `container-logs` in `src/backend/modules/workflows/service.go`.
Category: `action`.
