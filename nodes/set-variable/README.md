...
Store a value on the flow context
...
- `Variable Name` (text, required): Single-line text value.
...
This frontend node is bound to the workflow action key `set-variable` in `src/backend/modules/workflows/service.go`.
Category: `utility`.
