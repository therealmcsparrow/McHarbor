...
Remove containers that belong to a compose stack
...
- `Stack Name` (text, required): Single-line text value.
- `Remove Volumes` (toggle, optional): Turns behavior on or off. Default: `False`.
...
This frontend node is bound to the workflow action key `stack-remove` in `src/backend/modules/workflows/service.go`.
Category: `action`.
