...
Remove unused Docker resources from an environment
...
- `Resource` (select, optional): Choose one of the built-in options. Default: `all`. Options: All, Containers, Images, Volumes, Networks.
...
This frontend node is bound to the workflow action key `docker-prune` in `src/backend/modules/workflows/service.go`.
Category: `action`.
