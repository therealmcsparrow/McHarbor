...
- `Host` (text, required): Single-line text value.
- `Timeout (seconds)` (number, optional): Numeric value. Default: `5`.
...
This frontend node is bound to the workflow action key `ping` in `src/backend/modules/workflows/service.go`.
