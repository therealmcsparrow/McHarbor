...
- `Show All` (toggle, optional): Turns behavior on or off. Default: `True`.
- `Name Filter` (text, optional): Single-line text value.
...
This frontend node is bound to the workflow action key `container-list` in `src/backend/modules/workflows/service.go`.
Category: `action`.
