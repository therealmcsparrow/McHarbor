...
- `Name` (text, optional): Single-line text value.
- `Ports` (text, optional): Single-line text value.
- `Environment Variables` (key-value, optional): Build an object as key/value pairs.
- `Restart Policy` (select, optional): Choose one of the built-in options. Default: `no`. Options: No, Always, On Failure, Unless Stopped.
- `Auto Start` (toggle, optional): Turns behavior on or off. Default: `True`.
...
This frontend node is bound to the workflow action key `container-create` in `src/backend/modules/workflows/service.go`.
Category: `action`.
