...
Tag an image with a new target reference
...
- `Source Image` (text, required): Single-line text value.
- `Target Image` (text, required): Single-line text value.
...
This frontend node is bound to the workflow action key `image-tag` in `src/backend/modules/workflows/service.go`.
Category: `action`.
