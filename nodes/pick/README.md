...
Keep only selected fields on an object
...
- `Fields` (text, required): Single-line text value.
...
This frontend node is bound to the workflow action key `pick` in `src/backend/modules/workflows/service.go`.
Category: `utility`.
