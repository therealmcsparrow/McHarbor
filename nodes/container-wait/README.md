...
Wait for a container to reach a condition
...
- `Condition` (select, optional): Choose one of the built-in options. Default: `not-running`. Options: Not Running, Next Exit, Removed.
- `Timeout (seconds)` (number, optional): Numeric value. Default: `60`.
...
This frontend node is bound to the workflow action key `container-wait` in `src/backend/modules/workflows/service.go`.
Category: `action`.
