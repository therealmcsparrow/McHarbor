...
Send a GraphQL request and store the parsed response
...
- `Variables` (key-value, optional): Build an object as key/value pairs.
...
This frontend node is bound to the workflow action key `graphql-request` in `src/backend/modules/workflows/service.go`.
