...
Run a read-only SQL query against the workflow database
...
- `Query` (textarea, required): Multi-line text value for bodies, templates, or larger content.
...
Queries are limited to read-only access against workflow tables (`workflows`, `workflow_runs`, `workflow_kv`, `workflow_link_messages`, `workflow_metrics`) and result rows are capped for safety.

## Backend Binding

This frontend node is bound to the workflow action key `sql-query` in `src/backend/modules/workflows/service.go`.
Category: `utility`.
