...
Encode or decode a value with Base64
...
- `Action` (select, optional): Choose one of the built-in options. Default: `encode`. Options: Encode, Decode.
...
This frontend node is bound to the workflow action key `base64` in `src/backend/modules/workflows/service.go`.
Category: `utility`.
