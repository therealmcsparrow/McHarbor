...
Write a value to the workflow key-value store
...
- `TTL (seconds)` (number, optional): Numeric value. Default: `0`.
...
This frontend node is bound to the workflow action key `kv-set` in `src/backend/modules/workflows/service.go`.
Category: `utility`.
