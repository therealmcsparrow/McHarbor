...
Allow or block a message using a condition
...
- Output `block`: Sends the outgoing msg on this port.
...
This frontend node is bound to the workflow action key `filter` in `src/backend/modules/workflows/service.go`.
Category: `logic`.
