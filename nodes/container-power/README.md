...
Enable or disable a container using the power helper action
...
- `Action` (select, required): Choose one of the built-in options. Options: Enable, Disable.
...
This frontend node is bound to the workflow action key `container-power` in `src/backend/modules/workflows/service.go`.
Category: `action`.
