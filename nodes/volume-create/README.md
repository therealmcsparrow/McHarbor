...
- `Driver` (text, optional): Single-line text value. Default: `local`.
- `Labels` (key-value, optional): Build an object as key/value pairs.
...
This frontend node is bound to the workflow action key `volume-create` in `src/backend/modules/workflows/service.go`.
Category: `action`.
