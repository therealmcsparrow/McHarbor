...
- `Force` (toggle, optional): Turns behavior on or off. Default: `False`.
- `Prune Children` (toggle, optional): Turns behavior on or off. Default: `True`.
...
This frontend node is bound to the workflow action key `image-remove` in `src/backend/modules/workflows/service.go`.
Category: `action`.
